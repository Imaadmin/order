...
//                }
    ...
    var Upload = {
        ...
        uploadComplete: function (r) {
            ...
            try {
                var json = eval('(' + r + ')');//
                Base.callback(me.editor, me.dialog, json.data.url, json.status);
        ...
        submit: function (callback) {
            ...
            $(me.dialog).delegate(".edui-image-file", "change", function (e) {
                ...
                $('<iframe name="up"  style="display: none"></iframe>').insertBefore(me.dialog).on('load', function () {
                    ...
                    if (r == '') return;
                    r = r.substring(r.indexOf('">{"') + 2, r.length);
                    r = r.substring(0, r.indexOf('</pre>'));
                    me.uploadComplete(r);
                    $(this).unbind('load');
                    $(this).remove();
                ...
                me.dialog.find('.edui-image-content').on('drop', function (e) {
                    ...
                    $.each(fileList, function (i, f) {
                        if (/^image/.test(f.type)) {
                            ...
                            var turl = me.editor.getOpt('imageUrl') + "?type=ajax" + "&uid=" + JSON.parse(window.sessionStorage.getItem("userinfo")).id + "&token=" + JSON.parse(window.sessionStorage.getItem("userinfo")).token;
